...
def send_msg_all(pid,msg,client):
    for client,client_info in clients.items():
        print(f'sneding {msg} to {client_info}')
        with Client(address=(client_info['adress'], client_info['port']), 
...
def main(ip_adress):
    with Listener(address=(ip_address, 6000),authkey=b'secret') as listener:
